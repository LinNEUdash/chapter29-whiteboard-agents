...
**Courses:** I ran `outcomes` to generate Bloom's Taxonomy-aligned learning outcomes. The tool produced five outcomes, which I refined. The key adjustment: Courses initially proposed "Describe the whiteboard method" (Remember level), which I elevated to "Apply the whiteboard decomposition method to derive agent boundaries" (Apply level) — the chapter teaches a procedure, not just a concept.

**The Human Decision Node in detail:** The most important tool correction was in the AI scaffold. When I first ran the simulated LLM clustering, I had it produce a reasonable grouping. But this made the Human Decision Node trivial — there was nothing to reject. I deliberately redesigned the AI output to make the specific error that LLMs commonly make: grouping by semantic category ("retrieval" vs. "action") rather than by data flow. This error is realistic (LLMs are trained on text about categories, not on production traffic patterns) and architecturally significant (it splits CRM read/write across agents). My rejection — "semantic similarity ≠ data affinity" — is visible in the notebook and demonstrated on camera in the video.
